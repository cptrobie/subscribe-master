...
Application secrets — database credentials, the Stripe API key, the cbu.uz API key, the JWT RSA signing key pair, and anything else sensitive — are retrieved from HashiCorp Vault at runtime, not read from `.env` files or hardcoded config. This applies uniformly across every environment, including local development, rather than having local dev diverge from how staging/prod actually retrieve secrets. That consistency is the whole point: a secrets-handling bug that only shows up in production because local dev never exercised the real path is exactly the kind of gap this choice is meant to close.

**On the JWT signing algorithm (RS256, asymmetric, not HS256/HMAC):** chosen specifically ahead of a planned future modulith migration to multiple independently-deployable builds. Under HMAC, every module that needs to *verify* a token would need the same shared secret that's *also* capable of signing forged tokens — a real trust/blast-radius problem once more than one deployable unit exists. Under RSA, only the public key needs distributing to verifying modules; the private signing key stays confined to whichever module actually issues tokens. See `AuthService`'s class Javadoc for the fuller history of the JWT pivot itself (this project originally used opaque, DB-backed session tokens before discovering `task.pdf`'s literal requirement).
...
- **No formal risk register or DAR (Decision Analysis and Resolution) artifact.** A CMMI-style assessment flagged this as a gap. Deliberately not adopted: this project already does the actual substance of DAR — real alternatives considered, real trade-offs weighed, real outcomes recorded — inline, next to the decision it explains (see, e.g., the 2FA mechanism choice in `login_2fa_sequence.md`, the citext-vs-normalization discussion in `RegistrationRequest`, or the URL-vs-header versioning choice behind `NFR-25`). A separate, formal, weighted-criteria document would duplicate that same information in a second place — precisely the anti-pattern this project has avoided elsewhere (see §6.4's reasoning for why `notification_log`/`audit_logs`/plain logging stay separate by purpose rather than merged). Formal DAR exists to protect multi-stakeholder organizational decisions from disagreement or turnover; a solo project has neither problem to solve.

---

## 13. Module boundary strategy (planned, Wave 4.5 — not yet started)

### 13.1 Package-per-domain, enforced by Spring Modulith — not a multi-module build, not microservices

The scope is package-level module boundaries within the existing single Maven artifact and single Postgres instance: each domain's top-level package (`auth`, `customer`, `auditlog`, and whatever Waves 5+ add) becomes an enforced boundary — internals (repositories, entities) made package-private, cross-domain calls only through a deliberate API, verified automatically by Spring Modulith's `ApplicationModules.verify()` rather than left to convention. This deliberately stops short of a multi-module Maven build (separate `pom.xml` per domain) and stops well short of physical microservice extraction (separate deployables, separate databases, network calls between domains). Both are legitimate *next* steps if this project ever needed independent scaling or deployment of individual domains — neither is needed to get the actual benefit being sought here, which is compiler-enforced boundaries and a build-time check standing in for the code review a solo project doesn't get.

### 13.2 Why Wave 4.5, not right after the first violation was found, and not at the end

The first real cross-domain violation (`AuthService` → `CustomerRepository`/`Customer`) exists as early as Wave 1. The instinct to fix it immediately was deliberately overridden — see `IMPLEMENTATION_BACKLOG.md`'s Wave 4.5 rationale for the three reasons in favor of waiting until Wave 4 completes rather than acting the moment the violation was spotted. The honest cost of that choice, estimated against this project's actual current size (2 of 12 planned domains built) versus its size at the end of Wave 4 (~4 domains):

- **Without touching the database:** package reorg, visibility restriction, Modulith tooling, and the event-based decoupling convention — roughly 12–20 hours at Wave 1's size; roughly the same order of magnitude at Wave 4.5's size, since the *number* of domains needing a facade grows linearly, not the difficulty of building any one facade.
- **Including the database:** schema-ownership decisions, retrofitting existing tables' foreign keys, and re-verifying the full Testcontainers-based test suite — roughly an *additional* 15–20 hours at Wave 1's size, growing faster than linearly the longer this is deferred, since later domains' tables accumulate more real foreign-key relationships to unwind (subscriptions → customers, payments → subscriptions, etc.) the longer they're built without the boundary in place.

Waiting past Wave 4.5 — e.g. until Wave 11's quality gate — was rejected for the reason implied above: by then all 12 domains exist, coupling has had the maximum possible time to accumulate, and the retrofit risk (how much working code has to be touched at once to fix it) is at its highest, not its lowest. Wave 4.5 is the point where "enough real coupling exists to make this a genuine exercise" and "not so much that fixing it is a project-wide rewrite" both hold at once.

### 13.3 The specific violation this wave starts from

`AuthService` (package `auth`) directly injects `CustomerRepository` and constructs/reads `Customer` (package `customer`) — a JPA repository and entity, both `customer`-owned internals, used directly by another domain's business logic. The fix is a facade: `customer` exposes an explicit lookup API (a service method returning whatever `auth` actually needs — not the entity itself), `CustomerRepository` and `Customer` become package-private, and `AuthService` is refactored to call the facade instead. This is the template the wave's `NFR-26` issue generalizes to whatever equivalent violations exist by the time Wave 4 completes.
