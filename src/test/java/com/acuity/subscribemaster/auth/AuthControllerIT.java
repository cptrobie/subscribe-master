 ...
 * Testcontainers/Vault bootstrap pattern established in SubscribeMasterApplicationIT.
 * Exercises what AuthControllerTest/AuthServiceTest structurally cannot: real BCrypt
 * hash persistence and the real (not hand-built) Jackson/Bean Validation wiring. Does
 * NOT verify citext's case-insensitive matching -- email is normalized to lowercase
 * before reaching AuthService (see duplicateEmailDifferentCase_isRejectedByExistsByEmailCheck),
 * so this suite genuinely cannot confirm whether citext's DB-level behavior is correct.
 */
@Testcontainers
@SpringBootTest(
...
public class AuthControllerIT {
    ...
    void registerNewCustomer_persistsHashedPasswordAndAuditEvent() {
        ...
        ResponseEntity<RegistrationResponse> response =
                restTemplate.postForEntity("/auth/register", request, RegistrationResponse.class);
    ...
    void duplicateEmail_returnsConflictAndDoesNotDuplicateRow() {
        ...
        ResponseEntity<ApiError> secondAttempt =
                restTemplate.postForEntity("/auth/register", request, ApiError.class);

        assertThat(secondAttempt.getStatusCode()).isEqualTo(HttpStatus.CONFLICT);
        assertThat(secondAttempt.getBody()).isNotNull();
        assertThat(secondAttempt.getBody().code()).isEqualTo("USER_ALREADY_EXISTS");
    ...
    void duplicateEmailDifferentCase_isRejectedByExistsByEmailCheck() {
        ...
        ResponseEntity<ApiError> response =
                restTemplate.postForEntity("/auth/register", sameEmailDifferentCase, ApiError.class);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CONFLICT);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().code()).isEqualTo("USER_ALREADY_EXISTS");
    ...
    void malformedJsonBody_returnsBadRequest() {
        ...
        ResponseEntity<ApiError> response =
                restTemplate.postForEntity("/auth/register", entity, ApiError.class);
