 ...
 *
 * <p>Uses webEnvironment = RANDOM_PORT (rather than the default MOCK) so a real embedded web server
 * actually starts, letting this test make a genuine HTTP call rather than only confirming the
 * context boots. This requires spring-boot-starter-webmvc on the classpath (added in pom.xml
 * alongside springdoc-openapi-starter-webmvc-ui).
 *
 * <p>Spring Boot 4 moved TestRestTemplate from org.springframework.boot.test.web.client to
 * org.springframework.boot.resttestclient, requires the separate spring-boot-resttestclient (test)
 * and spring-boot-restclient dependencies, and no longer auto-configures the bean
 * under @SpringBootTest by default — @AutoConfigureTestRestTemplate is now required explicitly. See
 * the Spring Boot 4.0 migration guide.
 *
 ...
 * Testcontainers core, causing repeated dependency resolution failures. GenericContainer is part of
 * core itself, so it doesn't have that problem.
 */
@Testcontainers
@SpringBootTest(
